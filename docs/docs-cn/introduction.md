# GeaFlow简介
# GeaFlow起源
早期的大数据处理主要以离线处理为主，以Hadoop为代表的技术栈很好的解决了大规模数据的分析问题。然后数据处理的时效性不足，
...
<p>随着数据实时处理场景的丰富，尤其是在实时数仓的场景下，实时关系运算(即Stream Join)
...
<p>在蚂蚁的大数据应用场景中，尤其是金融风控、实时数仓等场景下，存在大量Join运算，如何提高Join
