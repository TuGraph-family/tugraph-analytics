...
package com.antgroup.geaflow.example.graph.statical.compute.labelpropagation;
...
public class LabelPropagation {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(LabelPropagation.class);

    public static final String REFERENCE_FILE_PATH = "data/reference/labelpropagation";

    public static final String RESULT_FILE_DIR = "./target/tmp/data/result/labelpropagation";
    ...
    public static IPipelineResult submit(Environment environment) {
        ...
        pipeline.submit((PipelineTask) pipelineTaskCxt -> {
            ...
            PWindowStream<IVertex<Integer, Integer>> result =
                ...
                .compute(new LabelPropagationAlgorithm(50))
    ...
    public static class LabelPropagationAlgorithm extends VertexCentricCompute<Integer, Integer, Integer, Integer> {

        public LabelPropagationAlgorithm(long iterations) {
            super(iterations);
        }

        @Override
        public VertexCentricComputeFunction<Integer, Integer, Integer, Integer> getComputeFunction() {
            return new LabelPropagationVCCFunction();
        }
    ...
    public static class LabelPropagationVCCFunction extends AbstractVcFunc<Integer, Integer, Integer, Integer> {
        ...
        public void compute(Integer vertexId, Iterator<Integer> messageIterator) {
            ...
            if (this.context.getIterationId() == 1) {
                ...
                Integer newLabel = Integer.MIN_VALUE;
                while (messageIterator.hasNext()) {
                    Integer next = messageIterator.next();
                    newLabel = Math.max(newLabel, next);
                }

                if (vertex.getValue() < newLabel) {
                    this.context.setNewVertexValue(newLabel);
                    sendMessage(newLabel);
                }
            }
        }

        private void sendMessage(Integer label) {
            for (IEdge<Integer, Integer> edge : this.context.edges().getOutEdges()) {
                this.context.sendMessage(edge.getTargetId(), label);
            }
        }
