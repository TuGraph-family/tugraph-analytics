...
public class ShortArrEncoder extends AbstractEncoder<short[]> {

    public static final ShortArrEncoder INSTANCE = new ShortArrEncoder();
    ...
    public void encode(short[] data, OutputStream outputStream) throws IOException {
        ...
        for (short datum : data) {
            Encoders.SHORT.encode(datum, outputStream);
        }
    }
    ...
    public short[] decode(InputStream inputStream) throws IOException {
        ...
        for (int i = 0; i < length; i++) {
            arr[i] = Encoders.SHORT.decode(inputStream);
        }
        return arr;
    }
