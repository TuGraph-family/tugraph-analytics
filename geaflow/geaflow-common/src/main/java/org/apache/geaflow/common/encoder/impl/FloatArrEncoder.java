...
public class FloatArrEncoder extends AbstractEncoder<float[]> {

    public static final FloatArrEncoder INSTANCE = new FloatArrEncoder();
    ...
    public void encode(float[] data, OutputStream outputStream) throws IOException {
        ...
        for (float datum : data) {
            Encoders.FLOAT.encode(datum, outputStream);
        }
    }
    ...
    public float[] decode(InputStream inputStream) throws IOException {
        ...
        for (int i = 0; i < length; i++) {
            arr[i] = Encoders.FLOAT.decode(inputStream);
        }
        return arr;
    }
