...
public class DoubleArrEncoder extends AbstractEncoder<double[]> {

    public static final DoubleArrEncoder INSTANCE = new DoubleArrEncoder();
    ...
    public void encode(double[] data, OutputStream outputStream) throws IOException {
        ...
        for (double datum : data) {
            Encoders.DOUBLE.encode(datum, outputStream);
        }
    }
    ...
    public double[] decode(InputStream inputStream) throws IOException {
        ...
        for (int i = 0; i < length; i++) {
            arr[i] = Encoders.DOUBLE.decode(inputStream);
        }
        return arr;
    }
