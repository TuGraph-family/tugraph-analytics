...
package com.antgroup.geaflow.common.errorcode;
...
public class ErrorFactory {
    ...
    public static String ERR_ID = "ERR_ID";
    public static String ERROR_MSG_PREFIX = "\n************\n";
    ...
    protected static void checkParam(Method method, Object[] args) {
        ...
        if (args == null || args.length == 0) {
            if ((format1.getFormatsByArgumentIndex() != null
                && format1.getFormatsByArgumentIndex().length > 0) || (
                format2.getFormatsByArgumentIndex() != null
                    && format2.getFormatsByArgumentIndex().length > 0)) {
                throw new AssertionError("mismatched parameter length between " + method.getName()
                    + " and its annotation @ErrCode");
            }
        } else {
            if ((format1.getFormatsByArgumentIndex() != null
                && format1.getFormatsByArgumentIndex().length > args.length)
                || format1.getFormatsByArgumentIndex() == null || (
                format2.getFormatsByArgumentIndex() != null
                    && format2.getFormatsByArgumentIndex().length > args.length)) {
                throw new AssertionError("mismatched parameter length between " + method.getName()
                    + " and its annotation @ErrCode");
            }
        }
                ...
                for (ValidationType validation : validations) {
                    switch (validation) {
                        case ANNOTATION_SPECIFIED:
                            if (anno1 == null || StringUtils.isEmpty(anno1.codeId()) || StringUtils
                                .isEmpty(anno1.cause())) {
                        ...
                        case ERROR_ID_CHECK:
                            if (anno1 == null) {
                                throw new AssertionError(String
                                    .format("error code method[%s]" + "has no @ErrId annotation!",
                                        method.getName()));
                            }
                            String errId = anno1.codeId();
                            if (!checkErrorCodeFmt(errId)) {
                                throw new AssertionError(String
                                    .format("error code method[%s]" + "has invalid error code: %s",
                                        method.getName(), errId));
                            }
                            if (errIds.contains(errId)) {
                                throw new AssertionError(String
                                    .format("error code method[%s]" + "has duplicated err id: %s",
                                        method.getName(), errId));
                            }
                            errIds.add(errId);
                            break;
                        ...
                        case ARGUMENT_MATCH:
                            if (anno1 == null) {
                                throw new AssertionError(String
                                    .format("error code method[%s]" + "has no @ErrCode annotation!",
                                        method.getName()));
                            }
                            ...
                            final Format[] formats =
                                msgFormats.length != 0 ? msgFormats : causeFormats;
                            ...
                            if (!(msgFormats.length != 0 && causeFormats.length != 0)) {
                                ...
                                for (int i = 0; i < formats.length; i++) {
                                    ...
                                    if (fmt1 instanceof NumberFormat) {
                                        e = paramType == short.class || paramType == int.class
                                                || paramType == long.class
                                                || paramType == float.class
                                                || paramType == double.class || Number.class
                                            .isAssignableFrom(paramType) ? paramType : Number.class;
                                ...
                                if (!types.equals(paramTypeList)) {
                                    throw new AssertionError(String.format("error code[%s]"
                                            ...
                                            + " format elements %s in annotation", method.getName(),
                                        types, paramTypeList));
                                }
                            }
                            break;
