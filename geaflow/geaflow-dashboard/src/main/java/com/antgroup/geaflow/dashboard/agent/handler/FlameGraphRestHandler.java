...
import com.antgroup.geaflow.dashboard.agent.model.FlameGraphRequest;
import com.antgroup.geaflow.dashboard.agent.model.FlameGraphType;
import com.antgroup.geaflow.dashboard.agent.util.DateUtil;
...
public class FlameGraphRestHandler {
    ...
    private static final String FLAME_GRAPH_FILE_SUFFIX = ".html";
    ...
    public FlameGraphRestHandler(String flameGraphProfilerPath, String agentDir) {
            ...
            if (fileList != null) {
                for (String f : fileList) {
                    ...
                    if (flameGraphFile.isFile() && f.startsWith(FLAME_GRAPH_FILE_PREFIX)
                        && f.endsWith(FLAME_GRAPH_FILE_SUFFIX)) {
    ...
    private void checkFlameGraphFilePath(String path) {
        Preconditions.checkArgument(
            path != null && path.startsWith(agentDir) && path.endsWith(
                FLAME_GRAPH_FILE_SUFFIX), "File path is invalid.");
    }
    ...
    private ProcessBuilder getCommand(FlameGraphRequest request) {
        ...
        filePath.append(agentDir).append("/").append(FLAME_GRAPH_FILE_PREFIX).append("-")
            ...
            .append("-").append(randomSuffix).append(FLAME_GRAPH_FILE_SUFFIX);
