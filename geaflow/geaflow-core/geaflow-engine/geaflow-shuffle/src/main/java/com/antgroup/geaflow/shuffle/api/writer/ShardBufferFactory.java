...
public class ShardBufferFactory {
    ...
    public static ShardBuffer getShardBuffer(DataExchangeMode dataExchangeMode,
                                             IConnectionManager connectionManager) {
        if (dataExchangeMode == DataExchangeMode.PIPELINE) {
            return new PipelineShardBuffer();
        }
        for (SpillableShardBuffer shardBuffer : ServiceLoader.load(SpillableShardBuffer.class)) {
