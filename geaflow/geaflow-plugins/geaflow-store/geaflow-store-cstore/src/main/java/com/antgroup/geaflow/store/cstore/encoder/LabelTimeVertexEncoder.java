...
public class LabelTimeVertexEncoder extends VertexEncoder {

    public LabelTimeVertexEncoder(GraphDataSchema dataSchema) {
        super(dataSchema);
    }
    ...
    public VertexContainer encode(IVertex vertex) {
        return new VertexContainer(
            ...
            ((IGraphElementWithLabelField) vertex).getLabel(),
    ...
    public IVertex decode(VertexContainer vertexContainer) {
        ...
        ((IGraphElementWithLabelField) vertex).setLabel(vertexContainer.label);
        ((IGraphElementWithTimeField) vertex).setTime(vertexContainer.ts);
        return emptyProperty || vertexContainer.property.length == 0 ? vertex :
               vertex.withValue(valueDeserializer.apply(vertexContainer.property));
    }
}