...
public class StepGlobalAggregateOperator extends AbstractStepOperator<StepAggregateFunction, VertexRecord, SingleValue> {

    private final IType<?> inputType;

    private Map<ParameterRequest, Object> requestId2Accumulators;

    public StepGlobalAggregateOperator(long id, IType<?> inputType, StepAggregateFunction function) {
        super(id, function);
    ...
    public StepOperator<VertexRecord, SingleValue> copyInternal() {
        return new StepGlobalAggregateOperator(id, inputType, function);
    }
}
