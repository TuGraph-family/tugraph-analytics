...
CREATE TABLE modern_vertex (
  ...
  type='file',
geaflow.dsl.file.path = '/tmp/geaflow-test/incr_modern_vertex.txt',
geaflow.dsl.window.size = 500
);
...
CREATE TABLE modern_edge (
  ...
  type='file',
geaflow.dsl.file.path = '/tmp/geaflow-test/incr_modern_edge.txt',
geaflow.dsl.window.size = 10
);
...
INSERT INTO tbl_result
  ...
  a_id, b_id, c_id
  FROM (
  MATCH (a:person) -[e:knows]->(b:person)-[e2:knows]->(c:person)<-[e3:knows]-(d:person) where d.id!=c.id
  RETURN a.id as a_id,b.id as b_id,c.id as c_id
  )
;
