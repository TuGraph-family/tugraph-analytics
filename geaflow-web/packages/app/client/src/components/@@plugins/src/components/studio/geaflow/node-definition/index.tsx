...
import {
  getNodeDefinitionList,
  deleteVerticeDefinition,
...
export const NodeDefinition: React.FC<{}> = ({}) => {
  ...
  const handelTemplata = async () => {
    const manageData = await getNodeDefinitionList(instanceName, {
      name: state.search,
    });
  ...
  const columns = [
    {
      title: $i18n.get({
        id: "openpiece-geaflow.geaflow.table-definition.VerticsName",
        dm: "点名称",
      }),
      dataIndex: "name",
      key: "name",
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            ...
            onConfirm={() => {
              deleteVerticeDefinition(instanceName, record?.name).then(
                (res) => {
                  if (res?.success) {
                    message.success(
                      $i18n.get({
        ...
        <>
          <Breadcrumb>
            <Breadcrumb.Item>
              ...
              >
                {$i18n.get({
                  id: "openpiece-geaflow.geaflow.table-definition.Vertices",
                  dm: "点定义",
                })}
              </a>
            </Breadcrumb.Item>
            ...
            <Breadcrumb.Item>
              {showCreatePage.readonly
                ? $i18n.get({
                    id: "openpiece-geaflow.geaflow.table-definition.VerticeDefinitionDetails",
                    dm: "点定义详情",
                  })
                : showCreatePage.currentItem
                ? $i18n.get({
                    id: "openpiece-geaflow.geaflow.table-definition.EditVerticeDefinitions",
                    dm: "编辑点定义",
                  })
                : $i18n.get({
        ...
        <div className={styles["definition"]}>
          <p>
            <span className={styles["definition-title"]}>
              {$i18n.get({
                id: "openpiece-geaflow.geaflow.table-definition.VerticesDefinition",
                dm: "点定义",
              })}
            </span>
            <span className={styles["meaing"]}>
              {$i18n.get({
                id: "openpiece-geaflow.geaflow.node-definition.DefineTheStructureOfThe",
                dm: "定义图依赖的点表结构。",
              })}
            </span>
          </p>
          <div className={styles["definition-table"]}>
            <div className={styles["definition-header"]}>
              <div className={styles["title"]}>
                {$i18n.get({
                  id: "openpiece-geaflow.geaflow.table-definition.VerticesDefinitionList",
                  dm: "点定义列表",
                })}
              </div>
              <div>
